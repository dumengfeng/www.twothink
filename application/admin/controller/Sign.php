...
class Sign extends Admin
{
    ...
    {
        $list = \think\Db::name('sign_up')->order('id asc')->paginate(2);
    ...
    public function edit($id = 0)
    {
        if ($this->request->isPost()) {
            ...
            $info = array();
            /* 获取数据 */
            $info = \think\Db::name('sign_up')->find($id);
            if (false === $info) {
                $this->error('获取配置信息错误');
            }
    ...
    public function del()
    {
        ...
        if(\think\Db::name('sign_up')->where($map)->delete()){
            //记录行为
            action_log('update_sign', 'sign', $id, UID);
            $this->success('删除成功');
        } else {
            $this->error('删除失败！');
        }
